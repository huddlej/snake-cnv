...
rule find_gaps_in_reference:
    ...
    shell: "python ~jhuddles/src/fasta_tools/find_fasta_gaps.py {input} | bedtools merge -i stdin -d 0 > {output}"
...
rule download_reads_by_run_and_number:
    ...
    params: url=_get_url_by_run_and_number
    shell: """ascp -QT -l 300m -i $(dirname $(dirname `which ascp`))/etc/asperaweb_id_dsa.openssh {params.url} {output}"""
